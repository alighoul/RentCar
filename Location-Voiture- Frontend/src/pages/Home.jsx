...
const Home = () => {
  ...
  // New state to store the filtered vehicles based on date selection
  ...
  // Function to fetch available vehicles based on selected date range
  const filterVehiclesByDate = async () => {
    const { dateDebut, dateFin } = reservationData;
    if (dateDebut && dateFin) {
      try {
        const response = await axios.post("http://localhost:3000/reservation/available", {
          dateDebut,
          dateFin,
        });
        setFilteredVehicles(response.data); // Set filtered vehicles based on date range
      } catch (error) {
    ...
    <Container>
      ...
      <Box sx={{ display: "flex", gap: 2, mb: 3 }}>
        ...
        <TextField
          ...
          InputLabelProps={{ shrink: true }}
        />
        <Button variant="contained" onClick={filterVehiclesByDate}>
          Filter Vehicles
        </Button>
      </Box>
